...
SELECT user_id, EXTRACT(MONTH FROM date_of_purchase) AS month, ROUND(SUM(amount_spent::numeric),0)
FROM purchases
WHERE date_of_purchase > CURRENT_DATE - INTERVAL '24 Months'
GROUP BY 1,2
HAVING ROUND(SUM(amount_spent::numeric),0) >= 10000
ORDER BY 1,2
...
--SQL Question 4: Average Duration of Employee's Service. Given the data on IBM employees, can you find the average duration 
--of service for employees across different departments? The Duration of service is represented as end_date - start_date. 
...
SELECT department,
	   ROUND(AVG(
       CASE 
	      WHEN end_date ISNULL THEN CURRENT_DATE - start_date
